...
// Server Component — gold / black / white palette

import Link from 'next/link';
import { Phone } from 'lucide-react';
import { buildWALink } from '@/lib/utils';

/* ── Inline mandala SVG — no file dependency ─────────── */
function InlineMandala({ className = '' }: { className?: string }) {
  ...
  return (
    <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg"
      ...
      <circle cx="100" cy="100" r="85" fill="none" stroke="currentColor" strokeWidth="0.3" strokeOpacity="0.35" strokeDasharray="3 3" />
      {spokes.map((s, i) => (
        <line key={i} x1="100" y1="100" x2={s.x2} y2={s.y2}
          stroke="currentColor" strokeWidth="0.25" strokeOpacity="0.2" />
      ))}
      {petals12.map((d, i) => (
        <path key={i} d={d} fill="currentColor" fillOpacity="0.07"
          stroke="currentColor" strokeWidth="0.4" strokeOpacity="0.35" />
      ))}
      {petals8.map((d, i) => (
        <path key={i} d={d} fill="currentColor" fillOpacity="0.12"
          stroke="currentColor" strokeWidth="0.5" strokeOpacity="0.4" />
      ))}
      <circle cx="100" cy="100" r="55" fill="none" stroke="currentColor" strokeWidth="0.4" strokeOpacity="0.25" strokeDasharray="4 4" />
      <circle cx="100" cy="100" r="38" fill="none" stroke="currentColor" strokeWidth="0.4" strokeOpacity="0.25" />
      <circle cx="100" cy="100" r="22" fill="none" stroke="currentColor" strokeWidth="0.5" strokeOpacity="0.3" />
      <circle cx="100" cy="100" r="8"  fill="currentColor" fillOpacity="0.3" />
      <circle cx="100" cy="100" r="13" fill="none" stroke="currentColor" strokeWidth="0.7" strokeOpacity="0.35" />
  ...
  return (
    <section className="cta-banner-section">
      ...
      <div className="absolute top-0 left-0 right-0 h-px z-10"
        style={{ background: 'linear-gradient(90deg, transparent, #DE9619, #F0B84A, #DE9619, transparent)' }}
        aria-hidden="true" />
