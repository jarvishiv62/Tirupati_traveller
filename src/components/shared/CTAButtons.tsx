...
export default function CTAButtons({
  ...
  return (
    ...
    >
      ...
      <a
        href={`tel:${phone}`}
        className={cn('btn-primary inline-flex items-center justify-center gap-2', sizeMap[size])}
      >
        <Phone size={18} strokeWidth={2} />
        <span>{phoneLabel ?? `Call ${displayPhone}`}</span>
      </a>
      ...
      <a
        ...
        className={cn('btn-whatsapp inline-flex items-center justify-center gap-2', sizeMap[size])}
      >
        {/* WhatsApp icon SVG (lucide doesn't have one) */}
        <svg
