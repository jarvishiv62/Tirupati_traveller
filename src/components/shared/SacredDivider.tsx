...
function MandalaDivider({
  ...
  const scale = behavior === "expand" ? 0.5 + ratio * 0.65 : 1.2 - ratio * 0.6;

  const opacity =
    behavior === "expand" ? 0.08 + ratio * 0.22 : 0.3 - ratio * 0.15;
  ...
  return (
    ...
    >
      ...
      <div className="absolute left-0 right-0 top-1/2 -translate-y-1/2 flex items-center z-0 px-8 pointer-events-none">
        ...
        <div
          style={{
            width: size === "sm" ? 80 : size === "md" ? 112 : 148,
            flexShrink: 0,
          }}
        />
        <div className="sacred-divider-line" />
...
export default function SacredDivider({
  ...
  if (variant === "wave")
    return <WaveDivider size={size} toColor={toColor} className={className} />;
  if (variant === "lotus")
    return <LotusDivider size={size} toColor={toColor} className={className} />;
  if (variant === "mandala")
    return (
      <MandalaDivider
        size={size}
